---
title: Quick exercise of designing a minimal shared state API in React
date: 2025-09-08
update: 2025-09-30
tags:
    - react
...
+   const [state, setState] = useStore(useContext(AppContext));
    ...
    const increasePopulation = () => {
      setState(state => ({ ...state, bears: state.bears + 1 }));
    };
...
Based on this approach and its minimalist spirit, I created [`@t8/react-store`](https://github.com/t8js/react-store#readme). It exports such `Store` and `useStore()` completing this exercise. In addition to the discussed, the package description covers its common use cases including a single-store and multistore setup (both being perfectly legit), filtering store state updates, with some live examples.
