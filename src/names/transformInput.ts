import {transform, type Config} from 'glyphmap';
...
export function transformInput(x: string, config: Config) {
    ...
    let partialOutput = '', output = '', uppercaseOutput = '';

    for (let i = 0; i < outputGlyphs.length; i++) {
        let inputGlyph = inputGlyphs[i], nextInputGlyph = inputGlyphs[i + 1];
        let outputGlyph = outputGlyphs[i];

        if (inputGlyph === STRESS_MARK)
            continue;
