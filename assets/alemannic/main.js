...
function initMap() {
    ...
    map.addEventListener('click', ({clientX: x0, clientY: y0}) => {
        ...
        let selectedMarkers = Array.from(map.querySelectorAll('.marker'))
            .filter(marker => {
                let {left, right, top, bottom} = marker.getBoundingClientRect();

                return x0 > left - dx && x0 < right + dx &&
                    y0 > top - dy && y0 < bottom + dy;
            });
    ...
    document.addEventListener('click', event => {
        if (!event.target.closest('#map, button.close'))
            renderPlaylist(null);
    });
}
...
function getMedia(markers) {
    ...
    for (let marker of markers) {
        ...
        for (let {title, author, url, source} of item.resources) {
            ...
            element.innerHTML = `<figcaption class="title">` +
...
Promise.all([loadMarkers(), loadPlaces()])
    .then(() => {
        // renderCountryLabels();
        renderPlaces();
        renderMarkers();
