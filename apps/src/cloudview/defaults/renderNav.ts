...
function setLink(key: string, href: string | undefined) {
  if (href === undefined || !container) return;
...
export function renderNav(ctx: Context) {
  if (!ctx.navContainer) return;

  if (!container) {
    container =
      ctx.navContainer instanceof Element
        ? ctx.navContainer
  ...
  if (ctx.mode === "list") {
    ...
    if (startIndex !== undefined && pageSize && total !== undefined) {
      ...
      setParam(
        "prev",
        "start",
        startIndex === 0 ? undefined : startIndex - pageSize,
      );
      setParam(
        "next",
        "start",
      ...
      setParam(
        "end",
        "start",
        startIndex < maxStartIndex ? maxStartIndex : undefined,
      );
    }
  }
