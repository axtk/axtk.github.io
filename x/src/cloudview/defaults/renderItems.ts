...
export function renderItems(ctx: Context) {
  if (!ctx.items?.length || !ctx.container) return;

  if (!container) {
    container =
      ctx.container instanceof Element
        ? ctx.container
  ...
  for (let item of ctx.items) {
    ...
    let fullViewURL =
      `?${getDefaultQuery("&")}k=${encodeURIComponent(item.index ?? "")}` +
      `&name=${encodeURIComponent(item.name ?? "")}`;

    if (item.id) element.id = item.id;

    if (item.name) element.dataset.name = item.name;
