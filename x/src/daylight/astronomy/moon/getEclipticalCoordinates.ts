...
import {MJD_J2000, ARCS} from '../const';
...
export function getEclipticalCoordinates(date: Date | number | string) {
    ...
    let L0 = frac(.606433 + 1336.855225*T);
    let l  = pi2*frac(.374897 + 1325.552410*T);
    let ls = pi2*frac(.993133 +   99.997361*T);
    let D  = pi2*frac(.827361 + 1236.853086*T);
    let F  = pi2*frac(.259086 + 1342.227825*T);
